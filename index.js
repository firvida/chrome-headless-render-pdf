...
class RenderPDF {
  ...
  static async generateMultiplePdf(pairs, options) {
    ...
    for (const job of pairs) {
      try {
        const buff = await renderer.renderPdf(
          job.url,
          renderer.generatePdfOptions()
        );
  ...
  async renderPdf(url, options) {
    ...
    await Emulation.setVirtualTimePolicy({
      ...
      budget: this.options.jsTimeBudget,
    });
  ...
  async spawnChrome() {
    ...
    if (this.commandLineOptions.windowSize !== undefined) {
      commandLineOptions.push(
        `--window-size=${this.commandLineOptions.windowSize[0]},${this.commandLineOptions.windowSize[1]}`
      );
    }
  ...
  async detectChrome() {
    ...
    throw Error(
      "Couldn't detect chrome version installed! use --chrome-binary to pass custom location"
    );
  }
  ...
  async checkChromeVersion() {
    ...
    try {
      ...
      if (version.product.search("/64.") !== -1) {
        ...
        this.error(
          "  This version is known to contain bug in remote api that prevents this tool to work"
        );
        this.error("  This issue is resolved in version 65");
        this.error(
          "  More info: https://github.com/Szpadel/chrome-headless-render-pdf/issues/22"
        );
      }
      this.log(
        `Connected to ${version.product}, protocol ${version.protocolVersion}`
      );
    } catch (e) {
      this.error(
        `Wasn't able to check chrome version, skipping compatibility check.`
      );
    }
  }
